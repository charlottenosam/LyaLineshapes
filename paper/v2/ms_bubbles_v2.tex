...
%DIF DEL ms_bubbles_submit.tex   Fri Apr 24 14:08:39 2020
%DIF ADD ms_bubbles.tex          Wed Aug 19 21:08:08 2020
%
...
%DIF < % \AtBeginDocument{\mathcode`v=\varv} % make v different from nu
%DIF -------
\usepackage{newtxtext,newtxmath} %DIF > 
\AtBeginDocument{\mathcode`v=\varv} % make v different from nu %DIF > 
%DIF -------
...
%DIF 53a53-55
 %DIF > 
% Please keep new commands to a minimum, and use \newcommand not \def to avoid %DIF > 
% overwriting existing commands. Example: %DIF > 
%DIF -------
\include{macros}
...
%DIF CMASON PREAMBLE %DIF PREAMBLE
\RequirePackage[dvipsnames]{xcolor} %DIF PREAMBLE
\RequirePackage[normalem]{ulem} %DIF PREAMBLE
\DeclareOldFontCommand{\bf}{\normalfont\bfseries}{\mathbf} %DIF PREAMBLE
\providecommand{\DIFadd}[1]{{\protect\color{Green} \bf #1}} %DIF PREAMBLE
\providecommand{\DIFdel}[1]{{\protect\color{lightgray} \footnotesize \sout{#1}}} %DIF PREAMBLE
...
\DIFaddbegin \DIFadd{The model provides a way to interpret the necessary conditions to observe a blue-shifted }\lya \DIFadd{peak emerging from a galaxy at $z\simgt6$. Of course, there are numerous scattering processes in the ISM, CGM and IGM which can absorb a blue peak at any redshift }\citep[e.g.,][]{Gunn1965,Zheng2010,Laursen2011}\DIFadd{, meaning that non-detection of a blue peak does not provide much information about any one of those media. However, in the rare cases where blue }\lya \DIFadd{flux }\textit{\DIFadd{has}} \DIFadd{made it through a relatively neutral IGM, this model demonstrates that constraints can be placed on the line-of-sight gas properties in front of the source, in particular, that for blue flux to have been detected at $z\simgt6$, the source must reside in a highly ionised region.
}
...
%
\BE \label{eqn:tau_GP}
\tau_\textsc{gp}(z_s) = \frac{f_\alpha \pi e^2}{m_e \nu_\alpha} \frac{\xHI n_\textsc{h}(z_s)}{H(z_s)} \approx 4.7\times10^5 \xHI \DIFaddbegin \DIFadd{\Delta }\DIFaddend \left(\DIFdelbegin \DIFdel{\frac{n_\textsc{h}}{\nh}}%DIFDELCMD < \right) \left(%%%
\DIFdelend \frac{1+z_s}{8}\right)^{3/2}.
\EE
%
...
\DIFdelbegin \DIFdel{To estimate }%DIFDELCMD < \Nion %%%
...
\Nion \approx \DIFdelbegin \DIFdel{5.2}%DIFDELCMD < \times10%%%
\DIFdelend \DIFaddbegin \DIFadd{\frac{3.3\times10^{54}}{\alpha} 10}\DIFaddend ^{\DIFdelbegin \DIFdel{55}\DIFdelend \DIFaddbegin \DIFadd{-0.4 (}\MUV \DIFadd{+ 20)}\DIFaddend }\left(\DIFdelbegin \DIFdel{\frac{1+z}{8}}%DIFDELCMD < \right) %%%
\DIFdel{\frac{10^{-0.4 (\MUV + 20)}}{\alpha(1+z)^{\beta + 2}}}%DIFDELCMD < \left(%%%
\DIFdelend \frac{912}{1500}\right)^{\beta + 2} \,\mathrm{s}^{-1}
\EE
%
...
\DIFadd{J_\mathrm{s} = }\fesc \DIFadd{\int_{\nu_\textsc{h}}^\infty \,}\dd\DIFadd{\nu \, \frac{L_\nu}{h\nu} \sigma_\mathrm{ion}(\nu)
}\EE
%DIF > 
...
\DIFadd{J_\mathrm{s} = }\fesc  \Nion \DIFadd{\frac{\alpha}{\alpha + 3} \sigma_{\mathrm{ion},0}
}\EE
%DIF > 
...
%DIF < 
}%DIFDELCMD < \BE \label{eqn:HII_gammasource}
%DIFDELCMD < %%%
\DIFdel{J_\mathrm{s} = }%DIFDELCMD < \fesc %%%
\DIFdel{\int_{\nu_H}^\infty \,}%DIFDELCMD < \dd%%%
\DIFdel{\nu \, \frac{L_\nu}{h\nu} \sigma_\mathrm{ion}(\nu)
}%DIFDELCMD < \EE
%DIFDELCMD < %%%
%DIF < 
\DIFdel{with the hydrogen photoionisation rate $\sigma_\mathrm{ion} = \sigma_{\mathrm{ion},0} (\nu/\nu_\textsc{h})^{-3}$, where $\sigma_{\mathrm{ion},0} \approx 6.3\times10^{-18}\textrm{cm}^2$ }%DIFDELCMD < \citep[e.g.,][]{Draine2011}%%%
\DIFdel{. Assuming as above an ionising spectrum $L_\nu \propto \nu^{-\alpha}$ yields
%DIF < 
}%DIFDELCMD < \BE \label{eqn:HII_gammasource_Nion}
%DIFDELCMD < %%%
\DIFdel{J_\mathrm{s} = }%DIFDELCMD < \fesc  \Nion %%%
\DIFdel{\frac{\alpha}{\alpha + 3} \sigma_{\mathrm{ion},0}
}%DIFDELCMD < \EE
%DIFDELCMD < %%%
%DIF < 
\DIFdel{where $\Nion$ is given by Equation~}%DIFDELCMD < \eqref{eqn:Nion_from_Muv}%%%
\DIFdel{.
}\DIFdelend 
...
%
\DIFdelbegin %DIFDELCMD < \begin{align} %DIFDELCMD < \label{eqn:opticallythin}%%%
%DIFDELCMD < %
%DIFDELCMD < R_\alpha =& \left(\frac{J_\mathrm{s}}{4\pi}\right)^{1/2} \left[ \frac{ \clump[2] \nh[2](z) \alpha_\textsc{b}(T)}{H(z) \tau_\mathrm{lim}}\frac{f_\alpha \pi e^2}{m_e \nu_\alpha} - \Gamma_\textrm{bg}(z) \right]^{-1/2} \\
%DIFDELCMD < %
%DIFDELCMD < \approx& 0.16 \, \left(\frac{\tau_\mathrm{lim}\Nion\fesc}{2.3 \times 5.2 \times 10^{55}\,\mathrm{s}^{-1}}\right)^{1/2}  \left(\frac{2.5\alpha}{\alpha+3}\right)^{1/2} \nonumber \\
%DIFDELCMD < &\times \left(\frac{\clump}{3}\right)^{-1}   \left(\frac{T}{10^4\,\mathrm{K}}\right)^{0.4} \left(\frac{1+z}{8}\right)^{-9/4} \,\mathrm{Mpc} \nonumber
%DIFDELCMD < %
%DIFDELCMD < \end{align}
%DIFDELCMD < %%%
...
Figure~\ref{fig:lya_transmission_minDV} shows the minimum \lya velocity offset\DIFaddbegin \DIFadd{, }\DV\DIFadd{, }\DIFaddend observable as a function of the distance to the first neutral region (\Rion) and residual neutral fraction in the bubble (\xHI). \DIFdelbegin \DIFdel{We }\DIFdelend \DIFaddbegin \DIFadd{This can be interpreted as the necessary conditions in the galaxy's surroundings for us to observe an emission line with given }\DV\DIFadd{. Our estimate does not depend on the intrinsic emission line shape, only on the transmission possible given the conditions inside the bubble. This means that our observations always provide lower limits on }\Rion \DIFadd{and upper limits on }\xHI \DIFadd{-- Figure~\ref{fig:lya_transmission_minDV} shows that if $\Rion=1$\,pMpc we could observe blue flux at -400}\kms\DIFadd{, however if a galaxy inside an ionized region of that size only }\textit{\DIFadd{emits}} \DIFadd{flux at -100}\kms \DIFadd{we infer $\Rion > 0.5$\,pMpc.
}

\DIFadd{To calculate the minimum }\DV \DIFadd{we }\DIFaddend compute \lya transmission \DIFdelbegin \DIFdel{$e^{\tau(\DV)}$ }\DIFdelend \DIFaddbegin \DIFadd{$e^{-\tau(\DV)}$ }\DIFaddend on a grid of \Rion and \xHI \DIFdelbegin \DIFdel{, enabling a model-independent estimate of the properties of an ionised bubble based on }%DIFDELCMD < \lya %%%
\DIFdel{transmission . We show the }\DIFdelend \DIFaddbegin \DIFadd{values as described in Section~\ref{sec:model_optdepth}. Some of the resulting transmission curves are shown in Figure~\ref{fig:lya_transmission}. For each pair of }\Rion \DIFadd{and }\xHI \DIFadd{values we compute the minimum }\DIFaddend velocity offsets observable if $>10\%$ of the flux emitted at that velocity offset is transmitted through the IGM. We choose 10\% as assuming an emitted \lya EW of 200\,\AA, this transmitted flux should be observable with current facilities. \DIFaddbegin \DIFadd{By using a grid of }\Rion\DIFadd{-}\xHI \DIFadd{values, our estimate does not assume any particular ionizing model (such as those described in Section~\ref{sec:model_HIIregion}), and thus provides a model-independent estimate of the properties of an ionised bubble based on }\lya \DIFadd{transmission.
}\DIFaddend 
